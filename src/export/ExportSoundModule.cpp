...
void ExportSoundModuleOptions::PopulateOrExchange(ShuttleGui & S)
{
   S.StartHorizontalLay(wxEXPAND, 0);
   {
      S.StartStatic(_("OrenoKoukaon Export Setup"), true);
	  {
		  ...
		  S.TieCheckBox(wxEmptyString, wxT("/FileFormats/SoundModuleSetRate"), true);

		  // StereoToMono
		  S.AddSpace(20, 0);
		  S.AddVariableText(_("StereoToMono"));
		  S.TieCheckBox(wxEmptyString, wxT("/FileFormats/SoundModuleStereoToMono"), true);

		  // Resample
		  S.AddSpace(20, 0);
		  S.AddVariableText(_("Resample"));
		  S.TieCheckBox(wxEmptyString, wxT("/FileFormats/SoundModuleResample"), true);

		  // Normalize
		  S.AddSpace(20, 0);
		  S.AddVariableText(_("Normalize"));
		  S.TieCheckBox(wxEmptyString, wxT("/FileFormats/SoundModuleNormalize"), true);

		  //// [TruncateSilence]

		  // Align
		  S.AddSpace(20, 0);
		  S.AddVariableText(_("Align"));
		  S.TieCheckBox(wxEmptyString, wxT("/FileFormats/SoundModuleAlign"), true);

		  // Flash
		  S.AddSpace(20, 0);
		  S.AddVariableText(_("Flash"));
		  S.TieCheckBox(wxEmptyString, wxT("/FileFormats/SoundModuleFlash"), true);
	  }
      S.EndStatic();
   }
   S.EndHorizontalLay();
                      ...
                      int WXUNUSED(subformat))
{
	...
	// stereo to mono
	if (gPrefs->Read(wxT("/FileFormats/SoundModuleStereoToMono"), true))
	...
	// resample
	if (gPrefs->Read(wxT("/FileFormats/SoundModuleResample"), true))
	...
	// normalize
	if (gPrefs->Read(wxT("/FileFormats/SoundModuleNormalize"), true))
	...
	// Align
	if (gPrefs->Read(wxT("/FileFormats/SoundModuleAlign"), true))
	{
		// Align Together
		{
			project->OnSelectAll();
	...
	// Flash
	if (gPrefs->Read(wxT("/FileFormats/SoundModuleFlash"), true))
	{
		...
		bool tooLong = false;
		int nTotal = 0;
		int const nMax = HEADER_COUNT_MAX_EX_V2;
		double t0 = DBL_MAX;
		double t1 = DBL_MIN;
		TrackListIterator iter(project->GetTracks());
		Track *t = iter.First();
		while (t) {
			if (t->GetSelected()) {
				...
				t0 = std::min(t0, t0x.back());
				t1 = std::max(t1, t1x.back());
				DWORD dwDataSize;
				if (nTotal <= HEADER_COUNT_MAX_EX_V1) {
					dwDataSize =
						SOUNDMODULE_SAMPLE_START_ADDRESS +
						(DWORD)floor((t1 - t0) * SOUNDMODULE_RATE + 0.5);
				}
				else {
					dwDataSize =
						HEADER_START_ADDRESS +
						sizeof(FlashDataHeader) * nTotal +
						sizeof(FlashDataHeaderInfo) +
						(DWORD)floor((t1 - t0) * SOUNDMODULE_RATE + 0.5);
				}
				if (dwDataSize > FLASH_DATA_SIZE) {
					tooLong = true;
					break;
				}
			}
